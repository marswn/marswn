...
echo -e "\033[0;00m"
echo -e "\e[36m"
./tg -s ./Marrson.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("BS", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S Marrson -X kill
screen -S Marrson ./Marrson
done
]])  
file:close() 
...
if tonumber(user_id) == tonumber(522708624) then
var = true  
...
var = 'البوت🤖'
elseif database:sismember(bot_id..'Sudo:User', user_id) then
var = database:get(bot_id.."Sudo:Rd"..msg.chat_id_) or 'االمطور 👨🏻‍🚒'  
elseif database:sismember(bot_id..'Basic:Constructor'..chat_id, user_id) then
var = database:get(bot_id.."BasicConstructor:Rd"..msg.chat_id_) or 'ممنشئ اساسي 🧑🏻‍🚀'
elseif database:sismember(bot_id..'Constructor'..chat_id, user_id) then
...
local Marrson_Msg = ''  
if msgs < 100 then 
Marrson_Msg = 'غير متفاعل' 
elseif msgs < 200 then 
Marrson_Msg = 'بده يتحسن' 
elseif msgs < 400 then 
Marrson_Msg = 'شبه متفاعل' 
elseif msgs < 700 then 
Marrson_Msg = 'متفاعل' 
elseif msgs < 1200 then 
Marrson_Msg = 'متفاعل قوي' 
elseif msgs < 2000 then 
Marrson_Msg = 'متفاعل جدا' 
elseif msgs < 3500 then 
Marrson_Msg = 'اقوى تفاعل'  
elseif msgs < 4000 then 
Marrson_Msg = 'متفاعل نار' 
elseif msgs < 4500 then 
Marrson_Msg = 'قمة التفاعل' 
elseif msgs < 5500 then 
Marrson_Msg = 'اقوى متفاعل' 
elseif msgs < 7000 then 
Marrson_Msg = 'ملك التفاعل' 
elseif msgs < 9500 then 
Marrson_Msg = 'امبروطور التفاعل☣️' 
elseif msgs < 10000000000 then 
Marrson_Msg = 'رب التفاعل💖'  
end 
return Marrson_Msg 
end
...
--------------------------------------------------------------------------------------------------------------
function SourceMarrson(msg,data) -- بداية العمل
if msg then
...
local keyboard = {
{'تعطيل التواصل ✖️','ضع كليشه ستارت 💬','تفعيل التواصل 🔛'},
{'ضع اسم للبوت ©','الاحصائيات 📈','المطورين 🕹'},
{'اذاعه بالتوجيه 🔖','قائمه العام 📜','اذاعه بالتوجيه خاص 👤'},
{'اذاعه 📌','تحديث السورس ™','اذاعه خاص 👤'},
{'تفعيل البوت الخدمي 🔓','جلب نسخه احتياطيه 📂','تعطيل البوت الخدمي 🔏'},
{"تنظيف الكروبات 📄",'حذف كليشه ستارت 🚫',"تنظيف المشتركين 👥"},
{'الغاء ✖'}
}
...
send(msg.chat_id_, msg.id_,Marrson_Msg) 
return false  
end 
if text then    
...
os.execute('rm -rf Marrson.lua')
os.execute('wget https://raw.githubusercontent.com/Marrson/Marrson/Marrson.lua')
...
send(msg.chat_id_, msg.id_,'📌¦ عدد المجموعات الان » { '..#group..' }'..Marrsonk..''..Marrson..'\n*📌┇ الان عدد المجموعات الحقيقي » { '..sendok..' } مجموعات\n')   
end
end
end,nil)
end
...
usertext = '\n👤¦ العضو » ['..result.title_..'](t.me/'..(username or 'DEV‌Marrson')..')'
...
usertext = '\n👤¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'
status  = '\n📫¦ تم ترقيته مطور في البوت'
...
local Marrson_Msg = database:get(bot_id.."Add:Filter:Rp2"..text..msg.chat_id_)   
if Marrson_Msg then    
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..Marrson_Msg.."] \n") 
else
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/DEV‌Marrson)}\n📛¦["..Marrson_Msg.."] \n") 
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
...
send(msg.chat_id_,0, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/DEV‌Marrson)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" ) 
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
os.execute('rm -rf Marrson.lua')
os.execute('wget https://raw.githubusercontent.com/Marrson/Marrson/master/Marrson.lua')
send(msg.chat_id_, msg.id_,'♻¦ تم تحديث السورس') 
dofile('Marrson.lua')  
end
...
Text = [[
تنصيب بوتك ع سورس ˹M𝒂𝒓𝒓𝒔𝒐𝒏࿅ راسلني♻️¦ 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
♻️¦ BOT @IZ77BOT
💻¦ MT   @GGGGGT 
📢¦ CH   @GGGGGT2
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  
send(msg.chat_id_, msg.id_,'👤¦ بواسطه ← ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'DEV‌Marrson')..') \n🔐¦ تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'👤¦ بواسطه ← ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'DEV‌Marrson')..') \n🔐¦ تـم قفـل التاك بالكتم ')  
end,nil)   
...
local UserName = (b.username_ or "Marrson")
...
if text == ("تاك المدراء") and Constructor(msg) then
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n👤¦ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'Marrson')..')'
statusk  = '\n📫¦ تم طرده من هنا'
...
usertext = '\n👤¦ المستخدم » ['..result.title_..'](t.me/'..(username or 'DEV‌Marrson')..')'
statusk  = '\n📫¦ تم طرده من هنا'
...
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
 usertext = '\n👤¦ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'
 statusk  = '\n📫¦ تم طرده من هنا'
send(msg.chat_id_, msg.id_, usertext..statusk)
else
 usertext = '\n👤¦ المستخدم » '..userid..''
...
time = times * 60
local function Restricted(Kara,Marrson)
if tonumber(Marrson.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, '\n⚠¦ عذرا لا تستطيع كتم ( '..Rutba(Marrson.sender_user_id_,msg.chat_id_)..' )')
return false  
end
if Can_or_NotCan(Marrson.sender_user_id_, msg.chat_id_) == true then
send(msg.chat_id_, msg.id_, '\n⚠¦ عذرا لا تستطيع كتم ( '..Rutba(Marrson.sender_user_id_,msg.chat_id_)..' )')
else
MuteUser(msg.chat_id_,Marrson.sender_user_id_,msg.date_+time)
local function name(kara,Marrson)
if Marrson.username_ == '' then name = Marrson.first_name_ else name = Marrson.username_ end
send(msg.chat_id_,msg.id_,"⚠ ¦ العضو《["..name.."](tg://user?id="..Marrson.sender_user_id_..")》\nا┄─━━◉━━─┄\n⏳¦ تم كتم لمدة "..times.." دقايق ⏰")
end
getUser(Marrson.sender_user_id_,name)
end
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
usertext = '\n👤¦ المستخدم » ['..result.title_..'](t.me/'..(username or 'Marrson')..')'
status  = '\n📥¦ تم حظره من المجموعه'
texts = usertext..status
chat_kick(msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, texts)
...
------------------------------------------------------------------------
if text == ("الغاء تقيد") or text == ("الغاء التقييد") and msg.reply_to_message_id_ and Mod(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
t = "\n📌¦ قائمة المنع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do  
local Marrson_Msg = database:get(bot_id.."Add:Filter:Rp2"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..Marrson_Msg..'}\n'    
end  
if #list == 0 then  
...
if blakrt == 'مميز 🕵🏻‍♀️' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n👤¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'..'\n📬¦ تم رفعه '..RTPA..' هنا\n')   
database:set(bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sender_user_id_,RTPA) 
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.sender_user_id_)  
elseif blakrt == 'ادمن 👷🏻‍♂️' and Manager(msg) then 
send(msg.chat_id_, msg.id_,'\n👤¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'..'\n📬¦ تم رفعه '..RTPA..' هنا\n')   
database:set(bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sender_user_id_,RTPA)
database:sadd(bot_id..'Mod:User'..msg.chat_id_,result.sender_user_id_)  
elseif blakrt == 'مدير 👮🏻‍♂️' and Constructor(msg) then
send(msg.chat_id_, msg.id_,'\n👤¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'..'\n📬¦ تم رفعه '..RTPA..' هنا\n')   
database:set(bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sender_user_id_,RTPA)  
database:sadd(bot_id..'Manager'..msg.chat_id_,result.sender_user_id_)  
...
database:srem(bot_id..'Manager'..msg.chat_id_,result.sender_user_id_)  
database:del(bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sender_user_id_)
elseif blakrt == 'مدير 👮🏻‍♂️' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n👤¦ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEV‌Marrson')..')'..'\n📬¦ تم تنزيله من '..RTPA..' هنا\n')   
end
end,nil)   
end   
...
if blakrt == 'مميز 🕵🏻‍♀️' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n👤¦ العضو » ['..result.title_..'](t.me/'..(text1[3] or 'DEV‌Marrson')..')'..'\n📬¦ تم رفعه '..text1[2]..' هنا')   
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.id_)  
...
send(msg.chat_id_, msg.id_,'['..Marrson_Msg[math.random(#Marrson_Msg)]..']') 
return false
end
...
if Sudo(msg) and not database:get(bot_id..'Left:Bot'..msg.chat_id_) and not SudoBot(msg) then 
...
🔰┓ م1 ← اوامر الحمايه 
🥇┛ م2 ← اوامر الادمنيه
📚┓ م3 ← اوامر¦ كتم¦ تقيد¦ الطرد
⚙️┛ م4 ← اوامر المدراء
🕹┓ م5 ← اوامر المنشئين
🎖┛ م6 ← اوامر المطورين
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉┉ 
🔖¦ CH ➥ [Marrson](t.me/Marrson)
]]
send(msg.chat_id_, msg.id_,Text) 
return false
end
...
Text = [[
🔰┓ اهلا بك√
⚜️┛ اوامر  بالكتم|بالتقييد|بالطرد
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
⚠️|قفل+الامر-بالكتم-بالتقييد-بالطرد
✅|فتح+الامر-بالكتم-بالتقيد-بالطرد
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
🔐|قفل التكرار (بالطرد/بالتقيد/بالكتم)

🔐|قفل السيلفي(بالطرد/بالتقيد/بالكتم)

🔐|قفل الماركداون(بالطرد/بالتقيد/بالكتم)

🔐|قفل الملفات (بالطرد/بالتقيد/بالكتم)

🔐|قفل التوجيه(بالطرد/بالتقيد/بالكتم)

🔐|قفل الاغاني (بالطرد/بالتقيد/بالكتم)

🔐|قفل الصوت (بالطرد/بالتقيد/بالكتم)

🔐|قفل الكيبورد(بالطرد/بالتقيد/بالكتم)

🔐|قفل الملصقات(بالطرد/بالتقيد/بالكتم) 

🔐|قفل المتحركه (بالطرد/بالتقيد/بالكتم)

🔐|قفل الالعاب (بالطرد/بالتقيد/بالكتم)

🔐|قفل الشارحه (بالطرد/بالتقيد/بالكتم)

🔐|قفل الصور (بالطرد/بالتقيد/بالكتم)

🔐|قفل الفيديو (بالطرد/بالتقيد/بالكتم)

🔐|قفل التاك (بالطرد/بالتقيد/بالكتم)

🔐|قفل المعرفات(بالطرد/بالتقيد/بالكتم)

🔐|قفل الروابط (بالطرد/بالتقيد/بالكتم)

🔐|قفل البوتات(بالطرد/بالتقيد/بالكتم)
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
🔖¦ CH ➥ [Marrson](t.me/Marrson)
]]
send(msg.chat_id_, msg.id_,Text) 
...
☣️┛ رفع /تنزيل← ادمن بكل الصلاحيات
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🏅┓ مسح ← المنشئين
🥇┛ مسح ← المدراء
♻️┓ مسح ← الادمنيه
...
Marrson = https.request("https://api.telegram.org/bot"..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local Json_Info = JSON.decode(Marrson)
...
--------------------------------------------------------------------------------------------------------------
SourceMarrson(data.message_,data')
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
------------------------------------------------------------------------
local Marrsonbot = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if Marrsonbot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..Marrsonbot.."] \n") 
